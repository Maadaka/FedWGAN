...
def args_parser():
    ...
    parser.add_argument('--learning_rate_img_private', type=float, default=1e-5, metavar='N',
                        help='learning_rate')  # 0.01
    ...
    parser.add_argument('--gpu', default=1, type=int, help="index of gpu")

    parser.add_argument('--n_critic', type=int, default=5,
                        help="num of epoch for discriminator")
    parser.add_argument('--clip_value', type=float, default=0.01,
                        help="ensure that the weights of the discriminator satisfy Lipschitz")
    ...
    parser.add_argument('--data_dir', type=str, default='FLICKR-25K.mat', help="name of dataset, default: './data/'")
...
class LocalUpdateDJSRH(object):
    ...
    def train_val_test(self, dataset, train_idxs):
        ...
        dataloader_train = {x: DataLoader(dataset[x], batch_size=self.args.batch_size,
                                          shuffle=shuffle[x], num_workers=0) for x in ['train']}

        return dataloader_train

    def update_weights_UCCH_new(self, args, idx, global_protos, model_img, model_txt, private_img_model,
                                private_txt_model, private_gen_model, private_disc_model, model_gen):
        # 损失函数
        ...
        global_gen_model = model_gen

        device = self.device

        global_img_model.to(device)
        global_txt_model.to(device)
        global_gen_model.to(device)

        private_img_model.to(device)
        private_txt_model.to(device)
        private_gen_model.to(device)
        private_disc_model.to(device)
        ...
        if args.optimizer == "sgd":
            optimizer_i_s = torch.optim.SGD(filter(lambda p: p.requires_grad, model_img.parameters()),
                                            lr=args.learning_rate_img)
            optimizer_t_s = torch.optim.SGD(filter(lambda p: p.requires_grad, model_txt.parameters()),
                                            lr=args.learning_rate_txt)
            optimizer_i_private = torch.optim.SGD(filter(lambda p: p.requires_grad, private_img_model.parameters()),
                                                  lr=args.learning_rate_img_private)
            optimizer_t_private = torch.optim.SGD(filter(lambda p: p.requires_grad, private_txt_model.parameters()),
                                                  lr=args.learning_rate_txt_private)
        else:
            optimizer_i = torch.optim.Adam(global_img_model.parameters(), lr=args.learning_rate_img,
                                           weight_decay=args.weight_decay)
            optimizer_t = torch.optim.Adam(global_txt_model.parameters(), lr=args.learning_rate_txt,
                                           weight_decay=args.weight_decay)
            optimizer_i_p = torch.optim.Adam(private_img_model.parameters(), lr=args.learning_rate_img,
                                             weight_decay=args.weight_decay)
            optimizer_t_p = torch.optim.Adam(private_txt_model.parameters(), lr=args.learning_rate_txt,
                                             weight_decay=args.weight_decay)
            # optimizer_G = torch.optim.Adam(private_gen_model.parameters(), lr=args.learning_rate_txt)
            # optimizer_D = torch.optim.Adam(private_disc_model.parameters(), lr=args.learning_rate_txt)
            optimizer_G = torch.optim.RMSprop(private_gen_model.parameters(), lr=0.00005)
            optimizer_D = torch.optim.RMSprop(private_disc_model.parameters(), lr=0.00005)


        criterion = nn.BCELoss()
        private_img_model.train()
        private_txt_model.train()
        private_gen_model.train()
        private_disc_model.train()
        ...
        for epoch in range(args.train_ep_private):
            for batch_idx, (img_F, txt_F, labels, index) in enumerate(self.dataloader_train):
                ...
                for _ in range(args.n_critic):
                    ...
                    fake_data = private_gen_model(z).to(device)
                    real_data = torch.concat((img, txt), dim=1).to(device)
                    ...
                    loss_cluster = self.cluster_contrast(hid_I_p, centroid.to(device),
                                                         peso_labels[-batch_size:].to(device),
                                                         batch_size) + \
                                   self.cluster_contrast(hid_T_p, centroid.to(device),
                                                         peso_labels[-batch_size:].to(device),
                                                         batch_size)
                    ...
                    real_output = private_disc_model(real_data).to(device)
                    fake_output = private_disc_model(fake_data.detach()).to(device)
                    loss_D = wasserstein_loss(real_output, fake_output)
                    loss = loss + loss_D
                    loss.backward()
                    optimizer_D.step()
                    optimizer_i_p.step()
                    ...
                    # 权重裁剪
                    weight_clipping(private_disc_model, args.clip_value)
                ...
                Lc = l_loss + ab_loss
                Lr = self.criterion(images_outputs, texts_outputs)
                loss = Lc * args.alpha + Lr * (1. - args.alpha) + GAN_loss
                ...
                loss_cluster = self.cluster_contrast(hid_I_p, centroid.to(device), peso_labels[-batch_size:].to(device),
                                                     batch_size) + \
                               self.cluster_contrast(hid_T_p, centroid.to(device), peso_labels[-batch_size:].to(device),
                                                     batch_size)
                ...
                fake_output = private_disc_model(fake_data).to(device)
        ...
        return global_img_model.cpu().state_dict(), global_txt_model.cpu().state_dict(), private_img_model.cpu().state_dict(), private_txt_model.cpu().state_dict(), \
...
class LocalTest(object):
    ...
    def test_split(self, args, dataloader, idcs_retrieval, idcs_query):
        ...
        dataloader = {x: DataLoader(dataset[x], batch_size=args.batch_size, shuffle=shuffle[x], num_workers=0) for x in
                      ['query', 'database']}
    ...
    def test_inference_twoway(self, idx, args, local_img_model, local_txt_model):
        ...
        re_BI, re_BT, re_L, qu_BI, qu_BT, qu_L = compress(self.databaseloader, self.testloader, model_img, model_txt,
                                                          device)
...
if __name__ == '__main__':
    ...
    for _ in range(args.num_users):
        ...
        private_gen_model = Generator(input_size=100, output_size=(4096 + txt_len))
        private_disc_model = Discriminator(input_size=(4096 + txt_len))

        private_img_model_list.append(private_model_img)
        private_txt_model_list.append(private_model_txt)
        private_gen_list.append(private_gen_model)
        private_disc_list.append(private_disc_model)
    ...
    global_gen = Generator(input_size=100, output_size=(4096+txt_len))
    ...
    for round in range(args.rounds):
        ...
        for idx in idxs_users:
            ...
            w_img, w_txt, private_w_img, private_w_txt, local_centroid = local_model.update_weights_UCCH_new(args, idx, global_avg_protos_for_this_client,
                                                                                                             model_img=copy.deepcopy(global_model_img),
                                                                                                             model_txt=copy.deepcopy(global_model_txt),
                                                                                                             private_img_model=copy.deepcopy(private_img_model_list[idx]),
                                                                                                             private_txt_model=copy.deepcopy(private_txt_model_list[idx]),
                                                                                                             private_gen_model=private_gen_list[idx],
                                                                                                             private_disc_model=private_disc_list[idx],
                                                                                                             model_gen = global_gen)
        ...
        if round % 2 == 0:
            with torch.no_grad():
                ...
                print('Test/MAPi2t@50/MAPt2i@50/user' + str(MAP_I2T_ave), MAP_T2I_ave,
                      round)  # MAPi2t/MAPt2i/ ,MAP_I2T_a_ave,MAP_T2I_a_ave
