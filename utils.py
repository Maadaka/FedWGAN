...
def get_embedding_Kmeans(corpus):
    ...
    # cluster_centers, cluster_assignment = clustering_model.fit(corpus_embeddings)
    cluster_assignment = clustering_model.fit_predict(corpus_embeddings)
...
def get_persolabels(number_of_clients,images):
    ...
    labels_cluster = one_hot(cluster_assignment.to(torch.int64), class_count=class_count)
